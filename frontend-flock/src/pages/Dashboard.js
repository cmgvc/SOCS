...
function Dashboard() {
    ...
    const getRequestedDisplayedCards = requestedMeetings.slice(startRequestedIndex, startRequestedIndex + 3);
    ...
    useEffect(() => {
        const fetchMeetings = async () => {
            try {
                ...
                const upcoming = data.filter(meeting =>
                    new Date(meeting.date) > currentTime &&
                    meeting.status !== 'Pending' && meeting.status !== 'Declined'
                );
                const past = data.filter(meeting => new Date(meeting.date) <= currentTime);
                const requested = data.filter(meeting =>
                    meeting.status === 'Pending' || (!isFaculty && meeting.status === 'Declined')
                );
        
                setUpcomingMeetings(upcoming.sort((a, b) => new Date(a.date) - new Date(b.date)));
                setPastMeetings(past.sort((a, b) => new Date(b.date) - new Date(a.date)));
                setRequestedMeetings(requested.sort((a, b) => new Date(a.date) - new Date(b.date)));
            } catch (error) {
        ...
        const interval = setInterval(() => {
            ...
            setRequestedMeetings(prevRequested => {
                const movedToPast = prevRequested.filter(meeting => new Date(meeting.date) <= currentTime);
                ...
                return prevRequested.filter(meeting => new Date(meeting.date) > currentTime);
            });
        }, 60000);
                        ...
                        <div className="dash-section">
                            ...
                            <div className="upcoming-panel">
                                {requestedMeetings.length > 0 && (
                                    <>
                                        <button onClick={handleRequestedPrevClick} disabled={startRequestedIndex === 0} className='panel-arrow'>
                                            <ArrowBackIosIcon />
                                        </button>
                                        <div className="meeting-cards">
                                        ...
                                        <button onClick={handleRequestedNextClick} disabled={startRequestedIndex + 3 >= requestedMeetings.length} className='panel-arrow'>
                                            <ArrowForwardIosIcon />
                                        </button>
                                    </>
                                )}
                            ...
                            {availabilities.length > 0 ? (
                                <ul>
                                    {availabilities.map((availability) => (
                                     <> <a href={availability.bookingUrl}>{availability.bookingUrl}</a><br /><br /></> 
                                    ))}
                                </ul>
                            ) : (
